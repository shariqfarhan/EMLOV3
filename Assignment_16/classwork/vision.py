...
def predict(inp_img: Image) -> Dict[str, float]:
    ...
    img_tensor = transform(inp_img).unsqueeze(
    ...
    with torch.no_grad():
        ...
        confidences = {categories[i]: float(
            probabilities[i]) for i in range(1000)}
...
if __name__ == "__main__":
    gr.Interface(
		...
		title=f"Classifier Endpoint from {socket.gethostname()}"
